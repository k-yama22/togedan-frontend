...
export const useTestAuth = () => {
  ...
  const testLogin = () => {
    ...
    axios
      .post(LOGIN_URL, {
        email: "togedan.test.user@gmail.com",
        password: "Testpass01",
      })
      .then((res) => {
        if (res.data) {
          console.log(res.data);
          lscache.set("accessToken", res.headers["access-token"], 1000);
          lscache.set("client", res.headers["client"], 1000);
          lscache.set("uid", res.headers["uid"], 1000);

          const resData = toCamelCaseObject(res.data.data);
          lscache.set("loginId", JSON.stringify(resData.id), 1000);
          lscache.set("loginImg", resData.image, 1000);
